...
if __name__ == '__main__':
    ...
    companies_cik_list_remove = [cik_mu.get_cik_for_symbol(symbol) for symbol in companies_symbol_list]

    sci = StocksSymbolInfoInterface(mt5_connector)

    sci_admissible_symbols = sci.compute_admissible_symbols(relative_cost_threeshold=-1.06,
                                                            investing_balance=1000,
                                                            in_market_days=15,
                                                            cost_threeshold_in_years=True)

    symbol_name_list = list(set(list(sci_admissible_symbols.index)) - {'PANW.NAS', })
    ...
    companies_cik_list_mt5 = symbols_match_df.cik.to_list()
    ...
    companies_cik_list = list(set(companies_cik_list) - set(companies_cik_list_remove))

    companies_cik_list = [cik for cik in companies_cik_list if cik in companies_cik_list_mt5]
    # ------------------------------------------------------------------------------
