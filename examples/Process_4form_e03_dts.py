...
year_list = list(range(2015, 2020, 1))
...
if False:
    ...
    sub_data = historical_data.data_df.loc[
        historical_data.data_df.index > tsymb_form4_df_ri_day.index[0], 'adjusted close']
...
print("Seems Good: {} \n ".format(((form4_df_ri_day_pos[label_eval] >= 0.0) * 1.).mean() > (
            (processed_form4_df_ri_day[label_eval] >= 0.) * 1.).mean()))
...
form4_df_ri_day_pos.groupby('CIK')[label_eval].mean()
form4_df_ri_day_pos.groupby('CIK')[label_eval].count()
